...
// 								10, _pUIFontTexMgr->GetFontHeight() + 4, 8, 4 );
	...
	// Strings
	m_cmbItemPlusEffect.AddString( _S( 1031, "�� ȿ�� �����" ) );		
	m_cmbItemPlusEffect.AddString( _S( 1032, "�� ȿ�� ����" ) );	
	...
	m_slbCharQuality.SetEnable( FALSE );	// �ӽ�
	m_slbCharQuality.SetCurPos( 4 );		// �ӽ�
	...
	m_slbFiltering.Create( this, 43, 263, 191, 5, 12, 12, 0, 7, 0, 6 );; // ���ֳ���� ��⶧���� ����ȿ�� �ܰ�ø�
	...
	// Strings
	m_cmbTargetDest.AddString( _S(4163, "��� �㰡" ) );		
	m_cmbTargetDest.AddString( _S(4164, "��ȯ��&�� �� ����" ) );
	m_cmbTargetDest.AddString( _S(4386, "ĳ����,��ȯ��&�� ����"));
	...
	int	nStrWidth = ( _S( 200, "�������̽�" ).Length() + 3 ) *
					( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
	//m_btnInitInterface.Create( this, _S( 194, "�ʱ�ȭ" ), OPTION_TEXT_SX + nStrWidth,
	...
	m_btnInitInterface.Create( this, _S( 194, "�ʱ�ȭ" ), 128, 335, 75, 22 );
	...
	// Cancel Button
	//m_btnCancel.Create( this, _S( 139, "���" ), 126, 345, 63, 21 );
	...
	m_btnCancel.Create( this, _S( 139, "���" ), 146, 366, 92, 21 );
...
#if defined (G_THAI)
	nStrWidth = FindThaiLen( _S( 198, "���� ä��" ) ) + tv_chat;	
#else
	nStrWidth = ( _S( 198, "���� ä��" ).Length() + tv_chat ) *
					( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#endif
	//m_cbtnEnterChat.Create( this, OPTION_TEXT_SX + nStrWidth , OPTION_ENTERCHAT_SY, 11, 11,
	//						_S( 198, "���� ä��" ), TRUE, nStrWidth, nStrWidth );
...
#else
	m_cbtnEnterChat.Create( this, 121, 144, 13, 13,
							_S( 198, "���� ä��" ), TRUE, nStrWidth - 15, nStrWidth );
#endif
...
#if defined (G_THAI)
	nStrWidth = FindThaiLen( _S( 738, "���� ����" ) ) + tv_auto;	
#else
	nStrWidth = ( strlen( _S( 738, "���� ����" ) ) + tv_auto ) *				
					( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#endif
	//m_cbtnAutoAttack.Create( this, OPTION_TEXT_SX + nStrWidth, OPTION_AUTOATTACK_SY, 11, 11,
	//							 _S( 738, "���� ����" ), TRUE, nStrWidth, nStrWidth );		
	...
	// ITS #6255 : ���þ� ��û���� ���� �ɼ� ��Ʈ�� �� ���� ����. [12/22/2011 rumist]
#if defined (G_THAI)
	nStrWidth = FindThaiLen( _S( 2118, "��ȯ �㰡" ) ) + tv_summon;	
#else
	nStrWidth = ( strlen( _S( 2118, "��ȯ �㰡" ) ) + tv_summon) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );	
#endif
	m_cbtnMovePermit.Create( this, 225, 144, 13, 13,
							_S( 2118, "��ȯ �㰡" ), TRUE, nStrWidth, nStrWidth );
...
#if defined (G_THAI)
	// TODO : ����� �� ���� ��ġ �� ���� �ҵ�...
	nStrWidth = FindThaiLen( _S(284, "����" ) ) + tv_summon;
#else
	nStrWidth = ( strlen( _S(284, "����" ) ) + tv_summon ) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );	
#endif
	m_cbtnHelp1Icon.Create( this, 225, 172, 13, 13,
								_S(284, "����" ), TRUE, nStrWidth, nStrWidth );
...
#if defined (G_RUSSIA)
	nStrWidth = ( strlen( _S( 4456, "�ŷ� �ź�" ) ) + tv_summon ) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#elif (G_THAI)
	nStrWidth = FindThaiLen( _S( 4456, "�ŷ� �ź�" ) ) + tv_summon;	
#else
	nStrWidth = ( strlen( _S( 4456, "�ŷ� �ź�" ) ) + tv_summon ) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#endif
	m_cbtnRejectExchange.Create( this, 225, 200, 13, 13,
							_S( 4456, "�ŷ� �ź�" ), TRUE, nStrWidth, nStrWidth );
	...
	// [7/9/2009 rumist] rejection request message.
#if defined (G_RUSSIA)
	nStrWidth = ( strlen( _S( 4457, "��Ƽ �ź�" ) ) + tv_summon - 1 ) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#elif (G_THAI)
	nStrWidth = FindThaiLen( _S( 4457, "��Ƽ �ź�" ) ) + tv_summon;	
#else
	nStrWidth = ( strlen( _S( 4457, "��Ƽ �ź�" ) ) + tv_summon ) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#endif
	m_cbtnRejectParty.Create( this, 225, 226, 13, 13,
							_S( 4457, "��Ƽ/������ �ź�" ), TRUE, nStrWidth, nStrWidth );
...
#ifdef	G_KOR
		nStrWidth = ( strlen( _S(5589, "����Ʈ�� �˾�") ) + 1 ) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#elif (G_THAI)
	nStrWidth = FindThaiLen( _S( 5589, "����Ʈ�� �˾�" ) ) + tv_summon;	
#else
		nStrWidth = ( strlen( _S(5589, "����Ʈ�� �˾�") ) + 3 ) * 
				( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#endif
	m_cbtnQuestBookOpen.Create(this, 121, 200, 13, 13, _S(5589, "����Ʈ�� �˾�"), TRUE, nStrWidth, nStrWidth);
...
#ifdef G_THAI
	nStrWidth = FindThaiLen( _S( 739, "��ü ȭ��" ) ) + tv_summon;
#else
	nStrWidth = ( strlen( _S( 739, "��ü ȭ��" ) ) + 1 ) *				
					( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#endif // (G_THAI)
...
#ifdef G_THAI
	nStrWidth = FindThaiLen( _S( 207, "���ȿ��" ) ) + tv_summon;
#else
	nStrWidth = ( _S( 207, "���ȿ��" ).Length() + 1 ) *
					( _pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing() );
#endif // (G_THAI)
...
static void OpenMainWindowFullScreen( PIX pixSizeI, PIX pixSizeJ )
{
	...
	// ITS # 6311 : ���þ� ��Ʈ�� ���� ��û. [12/15/2011 rumist]
#if defined (G_RUSSIA)
	swprintf( wchWindowTitle, L"���ѬԬ��٬ܬ�" );
#else
							...
							enum DisplayDepth eColorDepth, BOOL bFullScreenMode)
{
	...
	FinalizeShaders();	// Effect shader handle ����
	...
	if( bSuccess) {
		...
		// Date : 2005-09-22(���� 8:26:30), By Lee Ki-hwan
		// Ÿ��Ʋ �� �� ���� 
		...
		InitializeShaders(); // Effect shader handle ����			
		// report success
		return TRUE;
	}
...
//		m_cbtnFameOpen.Render();
	...
	{
		// Fullscreen check button
		// Date : 2005-01-15,   By Lee Ki-hwan : �븸���ö���¡
		/* Date : 2005-04-08(���� 9:50:11), By Lee Ki-hwan
			MDO ��û���� �߱��� ������ ��� ����, �븸 �����϶��� ������
		*/
		//if ( g_iCountry != TAIWAN  ) 
	...
	// Text in option
	//pDrawPort->PutTextEx( _S( 196, "�ɼ�" ), m_nPosX + OPTION_TITLE_TEXT_OFFSETX,
	//									m_nPosY + OPTION_TITLE_TEXT_OFFSETY, 0xFFFFFFFF );
	pDrawPort->PutTextExCX( _S( 196, "�ɼ�" ),
	...
	{
		pDrawPort->PutTextExCX( _S( 212, "�����" ),
												m_nPosX + OPTION_SUB_TITLE_TEXT_CX,
												m_nPosY + OPTION_SUB_TITLE_TEXT_CY, 0xE1B300FF );

		nX = m_nPosX + OPTION_TEXT_SX;
		pDrawPort->PutTextEx( _S( 213, "����� ����" ),
											//nX, m_nPosY + OPTION_BGM_SY, 0xF2F2F2FF );
											nX, m_slbBGM.GetAbsPosY() -20, 0xF2F2F2FF );									
		pDrawPort->PutTextEx( _S( 214, "ȿ���� ����" ),
